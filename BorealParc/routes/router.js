...
router.get('/', function(req, res, next) {
  User.find({isSuperAdmin:false},function(err, user){
    if(err)
      return done(err);
    else if(!user)
      res.render('index', {title: 'Accueil - Boréal Parc'});
    else 
      res.render('index', {title: 'Accueil - Boréal Parc', entreprise: user, isLog: req.user});  
  }).sort({companyName:1})
});
//////////////////////////
/// Section entreprise ///
...
router.get('/entreprise/:companyNameSlug', function(req, res, next) {
  User.findOne({ 'companyNameSlug' : req.params.companyNameSlug },function(err, user){
    if(err)
      return done(err);
    else if(!user)
      res.redirect('/');
    else res.render('entreprise', {title: user.companyName, entreprise: user, isLog: req.user});  
  })
})
/////////////////////////////////////
...
router.get('/dashboard', isSuperAdmin, function(req, res){
  ...
  .then(function(doc) {
    res.render('admin/dashboard.superadmin-home.hbs', {title: 'Dashboard SuperAdmin', message : req.flash('signupMessage'), isLog: req.user, items: doc});
  });
});
...
router.post('/dashboard/update', isSuperAdmin, function(req, res, next) {
  ...
  req.check('website','Le format du lien du site n\'est pas correct').optional({checkFalsy: true}).isURL();
  req.check('facebook','Le format du lien facebook n\'est pas correct').optional({checkFalsy: true}).isURL();
  req.check('twitter','Le format du lien twitter n\'est pas correct').optional({checkFalsy: true}).isURL();
  req.check('instagram','Le format du lien instagram n\'est pas correct').optional({checkFalsy: true}).isURL();
  req.check('leftIndicator','Le positionnement horizontal doit être un chiffre').optional({checkFalsy: true}).isInt();
  req.check('rightIndicator','Le positionnement vertical doit être un chiffre').optional({checkFalsy: true}).isInt();
  req.check('leftIndicator','Le positionnement horizontal doit être compris entre 0 et 100').optional({checkFalsy: true}).isIntRange(0,100);
  req.check('rightIndicator','Le positionnement vertical doit être compris entre 0 et 100').optional({checkFalsy: true}).isIntRange(0,100);
...
router.post('/dashboard/creation-compte-magasin', isSuperAdmin, function(req, res){
  ...
  req.check('password','Les mots de passe de sont pas identiques').isEqual(req.body.passwordVerification);
...
router.get('/dashboard/modification-elements-site', isSuperAdmin, function(req, res){
  res.render('admin/dashboard.superadmin-site-modification.hbs', {title: 'Modification élément site - Dashboard SuperAdmin', isLog: req.user, success: req.session.success, errors: req.session.errors});
  req.session.success = false;
  req.session.errors = null;
});
...
router.get('/dashboard/modification-mot-de-passe-superadmin', isSuperAdmin, function(req, res){
  res.render('admin/dashboard.superadmin-password.hbs', {title: 'Modification mot de passe - Dashboard SuperAdmin', isLog: req.user, success: req.session.success, errors: req.session.errors});
  req.session.success = false;
  req.session.errors = null;
});
...
router.get('/dashboard/modification-mot-de-passe-magasin', isLoggedIn, function(req, res){
  res.render('admin/dashboard.shopadmin-password.hbs', {title: 'Modification mot de passe - Dashboard Magasin', isLog: req.user, success: req.session.success, errors: req.session.errors});
  req.session.success = false;
  req.session.errors = null;
});
...
router.get('/*', function(req, res, next) {
  User.find({isSuperAdmin:false},function(err, user){
    if(err)
      ...
      res.render('E404', {title: 'Page Introuvable', message: 'Nous somme désolé mais la page que vous cherchez semble introuvable'});
    else 
      res.render('E404', {title: 'Page Introuvable', message: 'Nous somme désolé mais la page que vous cherchez semble introuvable', entreprise: user, isLog: req.user});  
  }).sort({companyName:1})
});
  ...
  isLoggedIn(req, res, function(){
    if(req.user.isSuperAdmin == true){
      return next();
        ...
        res.render('admin/dashboard.shopadmin-home.hbs', {title: 'Dashboard Magasin', isLog: req.user});
      }
  })
}
